...
let
  ...
  guiService.serviceConfig.ExecStart = lib.mkForce "${upgradeNotifyScript}";
  ...
  upgradeNotifyUserScript = pkgs.writeShellScript "myscript" ''
        ...
        ntfBase() { notify-send --urgency low --app-icon "folder-download-symbolic" --app-name "System Update" "$@"; }
        ...
        cleanup() {
          ...
          removeNtf;
          sleep 1;
          ntf --urgency critical "Update Completed...";
